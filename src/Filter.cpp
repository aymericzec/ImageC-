...
bool sortByName(const shared_ptr<Shape> a, const shared_ptr<Shape> b) { return a->perimeter() < b->perimeter();  }

void Filter::sortShapes(list<shared_ptr<Shape>> & shapes, function<bool(const shared_ptr<Shape> &, const shared_ptr<Shape> &)> f)
{

	shapes.sort([&](shared_ptr<Shape> & a, shared_ptr<Shape> & b) -> 
		bool {
...
void Filter::sortPerimeter(list<shared_ptr<Shape>> & shapes, bool increase)
{
	sortShapes(shapes, [](const shared_ptr<Shape> & a, const shared_ptr<Shape> & b) -> bool { return a->perimeter() < b->perimeter(); });
}

bool Filter::sortSurface(Shape & shape1, Shape & shape2, bool asc, Point p)
{
	if(asc)
		return (shape1.surface() < shape2.surface());
	else
		return (shape1.surface() > shape2.surface());
}

bool Filter::sortDistance(Shape & shape1, Shape & shape2, bool asc, Point p)
{
	if(asc)
		return (shape1.origineDistance(p) < shape2.origineDistance(p));
	else
		return (shape1.origineDistance(p) > shape2.origineDistance(p));
}


list<shared_ptr<Shape>> Filter::searchShape(list<shared_ptr<Shape>> & shapes, Shape * shape)
{
	
	list<shared_ptr<Shape>> search;
	
	for(auto it = cbegin(shapes); it != cend(shapes); it++)
	{
...
list<shared_ptr<Shape>> Filter::searchPerimeter(list<shared_ptr<Shape>> & shapes, const double perimeter)
{
